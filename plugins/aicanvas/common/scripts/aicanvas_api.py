#!/usr/bin/env python3
"""Small, dependency-free AICanvas HTTP and recovery-state helper."""
...
class NoRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):  # noqa: D401
        raise AICanvasError(f"拒绝 HTTP 重定向：{code} {newurl}")


def configured_host() -> str:
    """Use the public API by default, but never hide an invalid explicit override."""
    raw = os.environ["AICANVAS_HOST"] if "AICANVAS_HOST" in os.environ else DEFAULT_AICANVAS_HOST
...
def validate_host(raw: str) -> str:
    ...
    if not parsed.hostname or parsed.username or parsed.password:
        raise AICanvasError("AICANVAS_HOST 必须是无用户名/密码的绝对主机地址")
    loopback = parsed.hostname in {"localhost", "127.0.0.1", "::1"}
    if parsed.scheme != "https" and not (parsed.scheme == "http" and loopback):
        raise AICanvasError("AICANVAS_HOST 公网地址必须使用 https；仅 localhost/127.0.0.1/::1 可使用 http")
    if parsed.query or parsed.fragment or parsed.path not in ("", "/"):
        raise AICanvasError("AICANVAS_HOST 不能包含路径、查询参数或 fragment")
    return raw.rstrip("/")
...
def validate_api_path(path: str) -> str:
    ...
    if parsed.scheme or parsed.netloc or not parsed.path.startswith("/api/"):
        raise AICanvasError("请求目标必须是以 /api/ 开头的相对路径")
    return path


def read_json_argument(raw: str | None, file_path: str | None) -> Any:
    if raw and file_path:
        raise AICanvasError("--json 和 --json-file 只能使用一个")
    if file_path:
...
def request_json(
    ...
    *,
    ...
    if not anonymous:
        key = os.environ.get("AICANVAS_API_KEY", "")
        if not KEY_RE.fullmatch(key):
            raise AICanvasError("AICANVAS_API_KEY 缺失或格式不正确")
    ...
    try:
        ...
        raise AICanvasError(f"HTTP {exc.code}: {redact(raw)[:1000]}") from None
    except urllib.error.URLError as exc:
        raise AICanvasError(f"网络请求失败：{redact(str(exc.reason))}") from None
    if not 200 <= status < 300:
        raise AICanvasError(f"HTTP {status}: {redact(raw)[:1000]}")
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError:
        raise AICanvasError(f"服务端没有返回 JSON：{redact(raw)[:500]}") from None
    if isinstance(payload, dict) and payload.get("code") not in (None, 0):
        code = payload.get("code")
        stable = payload.get("error_code") or payload.get("errorCode") or payload.get("message")
        raise AICanvasError(f"灯虹业务错误 {code}: {redact(str(stable))}")
    return payload
...
def extract(payload: Any, dotted_path: str) -> Any:
    ...
    for part in dotted_path.split("."):
        if not isinstance(value, dict) or part not in value:
            raise AICanvasError(f"响应缺少必需字段：{dotted_path}")
        value = value[part]
    if value in (None, ""):
        raise AICanvasError(f"响应字段为空：{dotted_path}")
    return value
...
def load_state(path: pathlib.Path) -> dict[str, Any]:
    try:
        ...
        raise AICanvasError(f"状态文件不存在：{path}") from None
    if not isinstance(state, dict) or state.get("schema_version") != 1:
        raise AICanvasError("状态文件 schema_version 不受支持")
...
def command_submit(args: argparse.Namespace) -> None:
    ...
    if any(op.get("operation_id") == operation_id for op in state["operations"]):
        raise AICanvasError(f"operation_id 已记录，拒绝重复：{operation_id}")
    scope = json.loads(args.scope)
    if not isinstance(scope, dict):
        raise AICanvasError("--scope 必须是 JSON object")
    for mapping in args.resource or []:
        name, separator, field = mapping.partition("=")
        if not separator or not name:
            raise AICanvasError("--resource 必须是 name=response.path")
...
def command_state_init(args: argparse.Namespace) -> None:
    ...
    if path.exists() and not args.force:
        raise AICanvasError(f"状态文件已存在：{path}；拒绝覆盖")
    host = configured_host()
    atomic_write(
        path,
        {
...
def command_state_set(args: argparse.Namespace) -> None:
    ...
    if not separator or not key:
        raise AICanvasError("值必须是 key=value")
    target[key] = value
    state["updated_at"] = now_iso()
...
def command_record_operation(args: argparse.Namespace) -> None:
    ...
    if any(op.get("operation_id") == args.operation_id for op in state["operations"]):
        raise AICanvasError(f"operation_id 已记录，拒绝重复：{args.operation_id}")
    body = read_json_argument(args.json, args.json_file)
    scope = json.loads(args.scope)
    if not isinstance(scope, dict):
        raise AICanvasError("--scope 必须是 JSON object")
...
def command_update_operation(args: argparse.Namespace) -> None:
    ...
    if len(matches) != 1:
        raise AICanvasError(f"operation_id 应唯一存在：{args.operation_id}")
...
def main() -> int:
    try:
        ...
        args.func(args)
    except (AICanvasError, json.JSONDecodeError, OSError) as exc:
