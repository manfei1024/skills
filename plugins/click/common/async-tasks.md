...
Click 的生成任务不共用一种状态接口。**不能只记 task ID，也不能假设所有任务都能用 `/api/ai/tasks/{id}` 查询。**
...
`common/scripts/click_api.py` 统一处理 HTTPS Host、禁止重定向、JSON 编码、HTTP/业务错误、必需响应字段和原子状态写入。涉及 JSON 或扣费的请求优先使用它，不要临时拼 `curl -d`。

```bash
CLICK_API="<技能包根目录>/common/scripts/click_api.py"

# 普通查询；--expect 缺字段时直接失败
python3 "$CLICK_API" request GET "/api/ai/tasks/$TASK_ID" \
  --expect data.status
```

脚本只允许以 `/api/` 开头的相对路径，且只会把凭证发往经过校验的 Host：默认 `https://click.vibehub.art`，设置 `CLICK_HOST` 时使用覆盖值。它不会跟随重定向，也不会把 API Key 写进输出。
...
在用户当前工作目录创建 `CLICK-<项目名>.json`。不要手写或用文本替换更新；使用脚本原子写入：

```bash
python3 "$CLICK_API" state-init \
  --file "CLICK-夏日便利店.json" \
  --project-name "夏日便利店"
```
...
```bash
python3 "$CLICK_API" submit POST "/api/ai/image/task" \
  --state "CLICK-夏日便利店.json" \
  --operation-type media_task \
  --id-field data.id \
  --poll-path '/api/ai/tasks/{operation_id}' \
  --scope '{"kind":"image"}' \
  --estimated-credits 120 \
  --json-file /tmp/click-image-request.json
```
...
```bash
python3 "$CLICK_API" submit POST "/api/storyboards/auto-produce" \
  --state "CLICK-夏日便利店.json" \
  ...
  --json-file /tmp/click-auto-produce.json
```

### 恢复流程（BLOCKING）

1. 查找当前目录的 `CLICK-*.json`。
...
```bash
python3 "$CLICK_API" pending --file "CLICK-夏日便利店.json"
python3 "$CLICK_API" update-operation \
  --file "CLICK-夏日便利店.json" \
