...
description: |
  ...
  NOT for: 多集短剧、剧本/分镜/成片全流程 → click-drama。整理素材、合规预审核 → click-assets。
  不覆盖语音合成（TTS）、LLM 对话、音视频合成——这些暂无对外文档。
  Chain signal: 用户在这里试出满意的模型和参数后要做整片 → click-drama。参考素材要先过审 → click-assets。
---
...
JSON 请求与扣费提交使用 [`common/scripts/click_api.py`](../../common/scripts/click_api.py)。它负责安全编码、错误检查，并在返回控制权前把任务和正确轮询路径原子写入 `CLICK-*.json`。下文 curl 只用于说明端点和字段，不作为首选执行方式。
...
6. **鉴权门** —— 只有即将调用鉴权接口或提交生成时才检查 `CLICK_API_KEY`。缺失或格式不正确就让用户在运行 agent 的环境中配置并重启/刷新会话；不要让用户把密钥贴进对话。然后用 `GET /api/auth/me` 验证，`Unauthorized` 就停。
7. **扣费确认** —— 给出本次预估积分、团队账户扣费及不可中止/退款说明，等待用户明确确认后才提交。

`CLICK_HOST` 未设置时默认使用 `https://click.vibehub.art`。本地或私有部署必须设置带协议的完整 origin，例如 `http://127.0.0.1:8080` 或 `https://dev-click.example`；裸域名 `dev-click.example` 无效。所有 JSON HTTP 请求都通过共享脚本执行，不能把示例中的默认域名复制成实际请求从而绕过环境变量。
...
```bash
curl -s -X POST "$CLICK_HOST/api/ai/image/task" \
...
```bash
curl -s -X POST "$CLICK_HOST/api/ai/video/task" \
  -H "Authorization: Bearer $CLICK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "<来自 model-list>",
...
- [ ] 该模型 `review_asset_enabled=true` 时，参考素材已走审核素材库（→ [click-assets](../click-assets/SKILL.md)）
- [ ] 已把预估消耗念给用户，用户确认了
- [ ] 已告诉用户提交后无法中止
- [ ] `CLICK_API_KEY` 已在环境中配置并通过 `/api/auth/me` 验证
...
```bash
curl -s "$CLICK_HOST/api/ai/tasks/$TASK_ID" -H "Authorization: Bearer $CLICK_API_KEY"
```
...
```bash
curl -s -X POST "$CLICK_HOST/api/prompts/generate" \
  -H "Authorization: Bearer $CLICK_API_KEY" \
  -H "Content-Type: application/json" \
